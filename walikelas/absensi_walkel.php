...
$wali_kelas_id = $user_data['id_kelas'] ?? '';
...
// Jika user adalah wali kelas, atur filter kelas secara otomatis
if ($user_role === 'wali_kelas') {
    $kelas_filter = $wali_kelas_id;
}
...
while ($kelas_row = mysqli_fetch_assoc($kelas_result)) {
    $selected = ($kelas_filter == $kelas_row['id']) ? 'selected' : '';
    $kelas_options .= "<option value=\"" . htmlspecialchars($kelas_row['id'] ?? '', ENT_QUOTES, 'UTF-8') . "\" $selected>" . htmlspecialchars($kelas_row['kelas'] ?? '', ENT_QUOTES, 'UTF-8') . "</option>";
}

// Bangun query utama untuk mengambil data presensi
$query = "SELECT b.nama, k.kelas, a.tanggal, a.jam_masuk, a.jam_pulang,
          ...
          WHERE a.tanggal = '" . mysqli_real_escape_string($konek, $tanggal) . "' 
          AND a.jam_pulang IS NOT NULL";

// Tambahkan kondisi filter berdasarkan input user
if (!empty($search)) {
...
// Tambahkan kondisi filter kelas
    ...
    <div class="main-content">
        ...
        <div class="filter-container">
            <form method="GET" action="">
                <input type="text" id="search" name="search" placeholder="Cari Data" value="<?php echo htmlspecialchars($_GET['search'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                <input type="date" id="tanggal" name="tanggal" value="<?php echo htmlspecialchars($_GET['tanggal'] ?? date('Y-m-d'), ENT_QUOTES, 'UTF-8'); ?>">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    ...
                    <?php 
                            ...
                            echo "<tr><td colspan='7' style='text-align: center;'>Tidak ada data presensi yang ditemukan.</td></tr>";
                        }
                    ?>
                </tbody>
            </table>
