...
if (isset($_POST['btnSimpan'])) {
    ...
    $id_kelas = (isset($_POST['id_kelas'])) ? htmlspecialchars(trim($_POST['id_kelas'])) : null;
    ...
    // Jika validasi lolos, masukkan data ke dalam database
     $stmt = $konek->prepare("INSERT INTO users (username, password_hash, role, id_kelas) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("sssi", $username, $password_hash, $role, $id_kelas);
    ...
    $(document).ready(function() {
        ...
        function toggleKelasDropdown() {
            if (roleSelect.val() === 'wali_kelas') {
                ...
                kelasGroup.hide();
                // Reset nilai kelas saat disembunyikan
                $('#id_kelas').val('');
            }
        }
